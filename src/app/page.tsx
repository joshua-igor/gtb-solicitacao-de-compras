...
import { supabase, Store, Product, MOCK_STORES, MOCK_PRODUCTS } from '@/lib/supabase';
...
export default function OrderPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoadingData(true);
        // Safely querying the databases
        const { data: storesDb, error: storesErr } = await supabase.from('stores').select('*');
        const { data: productsDb, error: productsErr } = await supabase.from('products').select('*');
        ...
        console.warn('Usando mock-data para fallback offline ou banco sem tabelas criadas.');
        setStores(MOCK_STORES);
        setProducts(MOCK_PRODUCTS);
      } finally {
  ...
  // Math Calculations (Based on 7% Freight)
  const totalSorvetesQty = cart.reduce((sum, item) => sum + item.quantity, 0);
  const totalSorvetesValue = cart.reduce((sum, item) => sum + (item.product.price * item.quantity), 0);

  // Totals Breakdown
  const totalProduct = totalSorvetesValue * 0.85; // Example calculation for visual breakdowns
  const freight = totalSorvetesValue * 0.07; // 7% Flat corporate logistics freight
  const totalEstimated = totalProduct + freight + (totalSorvetesValue * 0.08); // Estimate with overheads
  const totalOrder = totalSorvetesValue;
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <header className="flex h-16 items-center justify-between border-b border-gray-200 bg-white px-8">
          ...
          <div className="flex items-center gap-4">
            <span className="text-xs text-gray-500 font-semibold bg-gray-100 py-1 px-3 rounded-full">
        ...
        <main className="flex flex-1 overflow-hidden p-6 gap-6">
          <div className="flex-1 bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col overflow-hidden">
            ...
            <div className="p-5 border-b border-gray-100 flex items-center justify-between gap-4 bg-gray-50/50">
              ...
              <div className="flex gap-2">
                <span className="rounded-md bg-orange-50 px-3 py-1.5 text-xs font-semibold text-orange-700 border border-orange-100">
                  Carga Gelada
                </span>
                <span className="rounded-md bg-gray-100 px-3 py-1.5 text-xs font-semibold text-gray-700">
                  SORVETES ({totalSorvetesQty})
                </span>
              </div>
            </div>
          ...
          <div className="w-80 bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col justify-between overflow-hidden">
            ...
            {/* Static Pricing Summaries inside right Panel matching visual reference */}
            <div className="p-4 bg-gray-50 border-t border-gray-100 space-y-3">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-[10px] font-bold text-gray-400 uppercase">Total em Sorvetes</p>
                  <p className="text-lg font-black text-gray-900">
                    R$ {totalSorvetesValue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] font-bold text-gray-400 uppercase">Total de Sorvetes</p>
                  <p className="text-lg font-black text-orange-600">{totalSorvetesQty}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <footer className="h-20 bg-white border-t border-gray-200 px-8 flex items-center justify-between">
          <div className="flex gap-8">
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1">
                Total Estimado <Info className="h-3 w-3 text-gray-400" />
              </p>
            ...
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1">
                Total do Pedido <Info className="h-3 w-3 text-gray-400" />
              </p>
              <p className="text-base font-extrabold text-gray-900">
            ...
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Total do Produto</p>
              <p className="text-base font-extrabold text-gray-900">
                R$ {totalProduct.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
              </p>
            </div>
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1">
                Frete <Info className="h-3 w-3 text-gray-400" />
              </p>
              <p className="text-base font-extrabold text-[#ff7a59]">
                R$ {freight.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} <span className="text-[10px] font-bold text-gray-400">(7%)</span>
              </p>
            </div>
          </div>
